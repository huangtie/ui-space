...
(function (factory) {
    ...
    CityPicker.prototype = {
        ...
        render: function () {
            var p = this.getPosition(),
                ...
                dropdown = '<div class="city-picker-dropdown" style="left:0px;top:100%;' +
                    ...
                    '<a class="active" data-count="province">一级菜单</a>' +
                    (this.includeDem('city') ? '<a data-count="city">二级菜单</a>' : '') +
                    (this.includeDem('district') ? '<a data-count="district">三级菜单</a>' : '') + '</div>' +
        ...
        feedVal: function () {
            this.$element.val(this.getClassID());
        },
        ...
        output: function (type) {
            ...
            var pushData = {'type':typeclass,'parent_id':parentCode};
            var url = "/admin.php?c=content&a=getClass";
            $.post(url,pushData,function (result) {
                var testData =  result.data;
                    $select.html(type === PROVINCE ? obj.getProvinceList(testData) :
                        obj.getList(testData, type));
        ...
        getProvinceList: function (data) {
            ...
            $.each(data, function (j, m) {
                list.push(
                    '<a' +
                    ' title="' + (m.class_name || '') + '"' +
                    ' data-code="' + (m.class_id || '') + '"' +
                    ' class="' +
                    (m.selected ? ' active' : '') +
                    '">' +
                    ( simple ? $this.simplize(m.class_name, PROVINCE) : m.class_name) +
                    '</a>');
            });
            list.push('</dd></dl>');
        ...
        getList: function (data, type) {
            ...
            $.each(data, function (i, n) {
                list.push(
                    '<a' +
                    ' title="' + (n.class_name || '') + '"' +
                    ' data-code="' + (n.class_id || '') + '"' +
                    ' class="' +
                    (n.selected ? ' active' : '') +
                    '">' +
                    ( simple ? $this.simplize(n.class_name, type) : n.class_name) +
                    '</a>');
            });
            list.push('</dd></dl>');
